...
7. **Verify the generator is closed** before copying the manifest up:
   ...
   The box holds live production credentials from the moment the manifest lands.
8. **Teardown, mandatory:** tear the cohort down from the console, confirm `load_test:status` reports nil, destroy the resource group, and `kamal app boot` if memory ballooned — on ~1.1 GB spare a grown Puma worker does not necessarily shrink back.
